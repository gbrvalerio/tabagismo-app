...
describe('questions.repository', () => {
  ...
  describe('useQuestions', () => {
    ...
    it('should use correct query key with context', () => {
      const { result } = renderHook(() => useQuestions('onboarding'), {
        wrapper: createWrapper(),
      });
    ...
    it('should call db.select and filter by context', async () => {
      const { result } = renderHook(() => useQuestions('onboarding'), {
        wrapper: createWrapper(),
      });
    ...
    it('should use different query keys for different contexts', () => {
      const wrapper = createWrapper();
      const { result: result1 } = renderHook(() => useQuestions('onboarding'), { wrapper });
      const { result: result2 } = renderHook(() => useQuestions('daily_checkin'), { wrapper });
  ...
  describe('useSaveAnswer', () => {
    ...
    it('should perform atomic upsert via onConflictDoUpdate', async () => {
      ...
      await act(async () => {
        await result.current.mutateAsync({ questionKey: 'name', answer: '"John"' });
      });
    ...
    it('should include context in inserted values', async () => {
      const mockReturning = jest.fn().mockResolvedValue([{ id: 1 }]);
      const mockOnConflictDoUpdate = jest.fn(() => ({ returning: mockReturning }));
      const mockValues = jest.fn(() => ({
        returning: mockReturning,
      ...
      expect(mockValues).toHaveBeenCalledWith(
        expect.objectContaining({ context: 'onboarding' })
      );
    });
  ...
  describe('useDeleteDependentAnswers', () => {
    ...
    it('should delete answers for direct dependent questions', async () => {
      ...
      const mockExecute = jest.fn().mockResolvedValue(undefined);
      const mockWhere = jest.fn(() => ({ execute: mockExecute, all: mockAll }));
      const mockFrom = jest.fn(() => ({
        all: mockAll,
        where: mockWhere,
    ...
    it('should not delete any answers when no dependent questions exist', async () => {
      ...
      const mockFrom = jest.fn(() => ({
        all: mockAll,
        where: jest.fn(() => ({ execute: jest.fn(), all: mockAll })),
        orderBy: jest.fn(() => ({ all: mockAll })),
      }));
      (db.select as jest.Mock).mockReturnValue({ from: mockFrom });
  ...
  describe('useDeleteAllAnswers', () => {
    ...
    it('should call db.delete when mutated', async () => {
      const mockExecute = jest.fn().mockResolvedValue(undefined);
